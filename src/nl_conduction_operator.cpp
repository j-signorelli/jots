...
double NonlinearConductionOperator::DiffusivityCoefficient::Eval(ElementTransformation &T, const IntegrationPoint &ip)
{
    return k.Eval(T, ip)/(rho.Eval(T, ip)*C.Eval(T,ip));
}

double NonlinearConductionOperator::dDiffusivityCoefficient::Eval(ElementTransformation &T, const IntegrationPoint &ip)
{
...
double NonlinearConductionOperator::NeumannCoefficient::Eval(ElementTransformation &T, const IntegrationPoint &ip)
{
    return g.Eval(T, ip)/(rho.Eval(T, ip)*C.Eval(T,ip));
}

double NonlinearConductionOperator::dNeumannCoefficient::Eval(ElementTransformation &T, const IntegrationPoint &ip)
{
...
double NonlinearConductionOperator::BetaCoefficient::Eval(ElementTransformation &T, const IntegrationPoint &ip)
{
    double k_val = k.Eval(T, ip);
...
double NonlinearConductionOperator::dBetaCoefficient::Eval(ElementTransformation &T, const IntegrationPoint &ip)
{
    double k_val = k.Eval(T, ip);
    double dk_val = dk.Eval(T, ip);
    double rho_val = rho.Eval(T, ip);
...
:  TimeDependentOperator(f.GetTrueVSize(), time_),
   ...
   dg_over_rhoC(*neumann_coeff.GetCoeff(0), rho_prop, C_prop),
...
void NonlinearConductionOperator::Mult(const Vector &u, Vector &k) const
{
	// Compute:
	//    k_n = M^{-1}A(u_n,t_n)
	// for k
...
void NonlinearConductionOperator::ImplicitSolve(const double dt,
...
void NonlinearConductionOperator::Iterate(Vector& u)
{
   // Step in time
