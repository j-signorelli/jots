...
To maintain generality, the NFIs are written under the presumption that given coefficients $\lambda(u)$ and $\lambda'(u)$ are updated *externally* **every Newton iteration** prior to calls to `AssembleElementVector` and `AssembleElementGrad`. This allows usage of the same NFIs for different applications/coefficients. Additionally, this prevents any `MaterialProperty` coefficients from being updated unexpected from calls to `AssembleElementVector` or `AssembleElementGrad`.


To aid in this, a `JOTSNewtonSolver` was inherited from `NewtonSolver`. This class accepts "registered" `MaterialProperty` objects to be updated with the most recent solution every Newton iteration, which is done through overriding `NewtonSolver::ProcessNewState`. Notably, material properties presently only depend on the solution $u$ but Newton iterations can be called on $k$. To ensure that the *solution* $\vec{u}$ is sent to the material property objects, a `JOTS_k_Operator` was created. Any operator sent to a `JOTSNewtonSolver` that iterates on `k` must be of type `JOTS_k_Operator`. The `JOTSNewtonSolver` will automatically detect it and ensure that $\vec{u}_{n+1}=\vec{u}_n + \Delta t\vec{k}_{n+1}$ is sent to each `MaterialProperty` instead of $\vec{k}_{n+1}$.
