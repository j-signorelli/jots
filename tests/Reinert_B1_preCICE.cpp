...
int main(int argc, char **argv)
{
    ...
    if (color == 0)
    {
        ...
        exit = preCICE_Dummy_Analytical_Test("Reinert_B1_preCICE_config.xml", "Heat-Flux", "Temperature", vertices, Constant_Temp, Reinert_B1_Analytical_HF, subRank, subSize, subComm);
        if (exit == 1)
            MPI_Abort(MPI_COMM_WORLD, EXIT_FAILURE);
    }
    else
